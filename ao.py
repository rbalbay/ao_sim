...
zero_magnitude_flux = 2.5E10  # photon / s
stellar_magnitude = 0
...
spiders_width = 0.0075  # 7.5 mm
spiders = 4
spatial_resolution = wavelength / m1_diameter

dm_actuators = 22
dm_delta = 1e-3  # 1kHz
dm_leakage = 0.01  # leaky integrator
dm_gain = 0.5  # control loop gain
dm_rcond = 1E-3  # condition

wfs_integration_time = 1e-2  # wfs integration time
...
for i in np.linspace(0, 1, 101):
    ...
    plt.subplot(2, 4, 1)
    plt.title(f'PSF (λ={wavelength}, S_mag={stellar_magnitude})')
